---
name: fhevm-testing
description: fhEVM testing with Hardhat mock environment — fhevm.initializeCLIApi, createEncryptedInput, userDecrypt (v0.4.1), publicDecrypt, publicDecryptEbool, verifyReveal patterns
type: reference
---
...
await contract
  .connect(user)
  .castVote(enc.handles[0], enc.handles[1], enc.inputProof);
```
...
## Deprecated callback pattern note

```typescript
// requestDecryption callback tests are deprecated in this stack.
// Prefer Pattern 3 test flow:
// 1) requestReveal(...)
// 2) fhevm.publicDecrypt([...handles])
// 3) verifyReveal(...handles, abiEncodedClearValues, decryptionProof)
```
